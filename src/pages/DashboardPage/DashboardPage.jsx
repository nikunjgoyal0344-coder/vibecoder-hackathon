...
// SIDEBAR
// ============================================================================
function Sidebar({ page, onNavigate, streak, collapsed, onToggleCollapse, mobileOpen, onCloseMobile, userSession, onLogOut, theme, onToggleTheme }) {
  ...
  const displayEmail = userSession?.email || 'krinaji.mb@gmail.com';
  const displayName = userSession?.name || 'Sarthak';
  const displayInitial = userSession?.initial || 'S';
  ...
  const content = (
    ...
    >
      ...
      <div className={`px-3 mb-2 ${collapsed ? 'px-2' : ''}`}>
        <div className={`flex items-center gap-3 bg-zinc-900/40 border border-zinc-800/80 rounded-lg p-2.5 ${collapsed ? 'justify-center' : ''}`}>
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500/30 to-violet-600/30 border border-purple-500/20 text-purple-200 flex items-center justify-center text-xs font-bold shrink-0">
            {displayInitial}
          </div>
          {!collapsed && (
            <div className="min-w-0 flex-1">
              <strong className="block text-xs text-zinc-200 truncate">{displayName}</strong>
              <span className="block text-[10px] text-zinc-500 truncate">{displayEmail}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="p-3 mt-2 border-t border-zinc-800 space-y-1">
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-zinc-500 hover:bg-zinc-900 hover:text-red-400 transition-colors ${
            collapsed ? 'justify-center px-0' : ''
          }`}
        >
...
function TowerOfHanoiGame() {
  ...
  return (
    <div>
      ...
      {won && (
        <div className="mb-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-4 py-3 flex items-center justify-between gap-3 max-w-lg h-56">
          <p className="text-sm text-emerald-300 flex items-center gap-2">
...
const LINKEDIN_NAME = 'Sarthak';
const LINKEDIN_HEADLINE = 'Computer Science Student';
const LINKEDIN_URL = 'https://www.linkedin.com/in/sarthak-profile';
...
function SettingsPage() {
  ...
  const [email, setEmail] = useState('krinaji.mb@gmail.com');
        ...
        <div className="bg-zinc-900/60 rounded-xl border border-zinc-800 p-4 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-zinc-500 mt-0.5">
              {darkMode ? 'Currently using the dark theme.' : 'Currently using the light theme.'}
            </p>
          </div>
          <button
            type="button"
            ...
            className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${darkMode ? 'bg-purple-500' : 'bg-zinc-700'}`}
          >
            <span
...
export default function Dashboard({ userSession, onLogOut, theme, onToggleTheme }) {
  ...
  const userEmail = userSession?.email || 'krinaji.mb@gmail.com';
  const isSarthak = userEmail === 'krinaji.mb@gmail.com';

  const [streak, setStreak] = useState(() => {
    const saved = localStorage.getItem(`abtalks_${userEmail}_streak`);
    return saved ? parseInt(saved, 10) : (isSarthak ? 7 : 0);
  });

  const [bestStreak, setBestStreak] = useState(() => {
    const saved = localStorage.getItem(`abtalks_${userEmail}_best_streak`);
    return saved ? parseInt(saved, 10) : (isSarthak ? 12 : 0);
  });

  const [marks, setMarks] = useState(() => {
    try {
      const saved = localStorage.getItem(`abtalks_${userEmail}_marks`);
      return saved ? JSON.parse(saved) : (isSarthak ? initialMarks : []);
    } catch (e) {
      return isSarthak ? initialMarks : [];
    }
  });

  const [challenges, setChallenges] = useState(() => {
    try {
      const saved = localStorage.getItem(`abtalks_${userEmail}_challenges`);
  ...
  const [logs, setLogs] = useState(() => {
    try {
      const saved = localStorage.getItem(`abtalks_${userEmail}_logs`);
      return saved ? JSON.parse(saved) : (isSarthak ? initialLogs : []);
    } catch (e) {
      return isSarthak ? initialLogs : [];
    }
  });

  const [assignments, setAssignments] = useState(() => {
    try {
      const saved = localStorage.getItem(`abtalks_${userEmail}_assignments`);
      return saved ? JSON.parse(saved) : (isSarthak ? initialAssignments : initialAssignments.map(a => ({ ...a, done: false })));
    } catch (e) {
      return isSarthak ? initialAssignments : initialAssignments.map(a => ({ ...a, done: false }));
    }
  });

  useEffect(() => {
    localStorage.setItem(`abtalks_${userEmail}_streak`, streak.toString());
  }, [streak, userEmail]);

  useEffect(() => {
    localStorage.setItem(`abtalks_${userEmail}_best_streak`, bestStreak.toString());
  }, [bestStreak, userEmail]);

  useEffect(() => {
    localStorage.setItem(`abtalks_${userEmail}_marks`, JSON.stringify(marks));
  }, [marks, userEmail]);

  useEffect(() => {
    localStorage.setItem(`abtalks_${userEmail}_challenges`, JSON.stringify(challenges));
  }, [challenges, userEmail]);

  useEffect(() => {
    localStorage.setItem(`abtalks_${userEmail}_logs`, JSON.stringify(logs));
  }, [logs, userEmail]);

  useEffect(() => {
    localStorage.setItem(`abtalks_${userEmail}_assignments`, JSON.stringify(assignments));
  }, [assignments, userEmail]);

  const activeLabel = nav.find((n) => n.id === page)?.label ?? page;

  return (
    <div className="flex h-screen font-sans relative overflow-hidden dash-text-primary dash-grid-bg">
      <Sidebar
        page={page}
        onNavigate={setPage}
        streak={streak}
      ...
      <main className="flex-1 flex flex-col min-w-0 dash-grid-bg">
        <div className="md:hidden flex items-center justify-between px-4 py-3 border-b border-zinc-800/80 bg-black">
          <div className="flex items-center gap-3">
            <button
              type="button"
          ...
          <div className="flex items-center gap-1.5">
            <button 
              ...
              className="p-1.5 rounded-md text-zinc-400 hover:text-purple-400"
              title="Toggle Theme"
            >
